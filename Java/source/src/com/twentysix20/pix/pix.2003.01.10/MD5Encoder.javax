...
public class MD5Encoder {
    ...
    /**
     * Get the encoding, or "digest", for the input stream.
     * This method constructs the input stream digest, and return it, as a
     * a byte array, following the MD5 (rfc1321) algorithm,
    ...
    /**
     * Get the encoding, or "digest", when the input is known to be a string.
     * This method doesn't throw any IOException, since it knows that the
     * underlying stream ws built from a String.
     * @exception RuntimeException This method will throw a RuntimeException if the
     *      	  input was not a string.
     */

    public byte[] getDigestFromString ()
    {
		if ( ! stringp )
			throw new RuntimeException (this.getClass().getName()
										+ "[getDigestFromString] Input was not a string.") ;
		try
		{
			return getDigest() ;
    ...
    /**
     * Calculates the "digest" and returns the results as a String.
     * @return A String representation of the message digest.
     * @exception IOException Thrown if the digestifier was unable to read the
     *    input stream.
     */
    public String getDigestAsString() throws IOException {
		return stringify (getDigest()) ;
    }
    /**
     * An implementation of Object's toString() method that calculates and returns the "digest" as a String.
     * @return A String representation of the message digest.
     * @exception IOException Thrown if the digestifier was unable to read the
     *    input stream.
     */
    public String toString() {
		try
		{
			return getDigestAsString();
		} catch (IOException e) {
			throw new RuntimeException (this.getClass().getName()
								+ ": toString() method threw IOException (trouble reading from InputStream): " + e.toString());
		}
    }
     ...
     * @param encoding the type of character data being digestified (such as UTF8)
     */

    public MD5Encoder (String input, String enc) {
 	byte bytes [] = null;
	try {
    ...
    /**
     * Construct a "digestifier" for the given string with the default character type of UTF8.  (UTF8 is good for ASCII data and normal Java Strings.)
     * @param input The string to be digestified.
     */
    ...
    public MD5Encoder (InputStream in) {
	this.stringp = false ;
	this.in      = in ;
	this.state   = new int[4] ;
	this.buffer  = new byte[64] ;
	...
	MD5Encoder md5 = new MD5Encoder (new FileInputStream(new File(args[0]))) ;
